...
> Auto-loaded by Claude Code. Contains everything needed for live work.
> Full reference (EQ maps, AbletonOSC patches, retired tools): `docs/CLAUDE-reference.md`
...
If this folder has moved, treat the folder containing `CLAUDE.md` as `AI_MUSIC_ROOT` and resolve local files relative to it. See `PORTABILITY.md`.
...
→ /opt/homebrew/bin/ableton-live-mcp → Claude Code MCP
...
+ PluginBridge MCP       (VST3/AU plugin, port auto-assigned — third-party plugin param control)
```

**Startup:** Prefs → Control Surface 1 = Ableton_Live_MCP | CS 2 = AbletonOSC (optional)
**Stale MCP:** `pkill -f ableton-live-mcp` → reconnect in Claude Code
**PluginBridge:** Load the VST3 on any track in Ableton → select channel name → pick plugin → MCP auto-connects
...
- PluginBridge `set_params` → the ONLY way to control Pro-Q 4, Serum, and other third-party plugins
...
Third-party plugin parameter control + real-time per-track audio analysis. Built as a VST3/AU plugin that loads inside Ableton and exposes any hosted plugin's parameters via MCP.

**Source:** `/Volumes/T7 Shield/Users/Aditya/Downloads/AI MUSIC PRODUCTION/pluginbridge/`
**Registered in Claude Code:** `pluginbridge` MCP (HTTP, auto-port per instance)
**Roadmap docs:** `AI MUSIC PRODUCTION/docs/PLUGIN_BRIDGE_ROADMAP.md`
...
4. MCP tools are live instantly — no other config
...
| Plugin | Before | After |
|---|---|---|
| Pro-Q 4 | ❌ 1 param via Ableton LOM | ✅ All 737 params |
| Serum 2, Diva, Omnisphere | ❌ locked | ✅ full control |
| Any third-party VST3/AU | ❌ locked | ✅ full control |

**Channel list (baked into dropdown):**
Vocals (Lead/Backing/Adlibs) · Drums (Kick/Snare/Hi-Hat/Overheads/Percussion)
Bass (808/Guitar/Sub/Synth) · Keys & Guitar · Synths · Strings & Brass · FX
Buses (Vocal/Drum/Guitar/Synth/Music/Instrument/Master) · Custom...

**Version:** v0.6.0 (Sprint 7 in progress — in-process param routing)
**Safe plugins** (load in-process, full GUI + spectrum): Pro-Q 4, The God Particle, Little MicroShift, LIMITER
**Unsafe plugins** (audio-only, no GUI): iZotope Ozone 12, Neutron 5 — MCP still works
...
**MCP wired in `.mcp.json`** — auto-loads on Claude Code startup via `start_mem0_mcp.sh`.
...
**Claude:** use `memory_search` at session start, `memory_add` after any decision that worked.
